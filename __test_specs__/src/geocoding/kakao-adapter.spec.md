...
### UC-LM-KAKAO-009 좌표가 숫자로 파싱되지 않으면 키워드 검색으로 넘어간다

- **Given** SDK 가 `OK` 와 함께 빈 좌표 문자열을 주고
- **When** `geocode` 를 부르면
- **Then** 키워드 검색이 한 번 불리고, 그쪽도 빈손이면 `ok: false` 다

`NaN` 좌표를 지도에 넘기면 마커가 사라지거나 엉뚱한 곳에 생긴다. **쓸 수 없는 항목은
결과가 없는 것과 같으므로 폴백을 그대로 탄다** — 쿼터를 한 건 더 쓰지만, 사용자가 친
문자열이 상호명이었을 가능성이 남아 있다.
...
### UC-LM-KAKAO-011 주소 검색이 OK 인데 결과가 비어 있으면 키워드로 넘어간다

- **Given** 주소 검색이 `OK` 와 함께 빈 배열을 주고
- **When** `geocode` 를 부르면
- **Then** 키워드 검색이 한 번 불린다

`OK` 인데 항목이 없는 응답이 실제로 온다. `ZERO_RESULT` 만 폴백 조건으로 두면 이 경우가
새어 나간다.

### UC-LM-KAKAO-012 키워드 검색의 ERROR 도 sdk 실패로 분류한다

- **Given** 주소 검색이 `ZERO_RESULT`, 키워드 검색이 `ERROR` 를 주고
- **When** `geocode` 를 부르면
- **Then** `failure.reason` 이 `sdk` 다

UC-LM-KAKAO-007 은 주소 검색만 본다. 두 번째 호출도 같은 분류를 따라야 한다.

### UC-LM-KAKAO-013 진행 중에 abort 되면 콜백을 기다리지 않고 끝난다

- **Given** 콜백을 영영 부르지 않는 SDK 가 있고
- **When** 조회 도중 `abort()` 하면
- **Then** `geocode` 가 값으로 끝난다

**이것이 없으면 멈춤 버튼이 듣지 않는다.** 콜백이 오지 않으면 프라미스가 settle 되지
않고, 큐의 `Promise.all` 도 끝나지 않아 그 줄이 `loading` 에 고정된다. 스크립트가 죽거나
응답이 유실되면 실제로 이렇게 된다.

### UC-LM-KAKAO-014 abort 보다 먼저 도착한 성공 결과는 살린다

- **Given** 콜백이 좌표를 돌려준 **뒤에** `abort()` 가 오고
- **When** `geocode` 의 결과를 기다리면
- **Then** `ok: true` 다

이미 쿼터를 쓴 응답이다. 버리면 사용자가 멈춘 대가로 좌표까지 잃는다. 큐도 같은 이유로
"성공은 살린다" 로 돼 있다.
