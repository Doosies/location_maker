...
화면에서는 셋 다 "지도가 안 뜬다" 로 같아 보이지만 사람이 할 일이 전혀 다르다 —
키를 넣거나, 네트워크를 보거나, 콘솔에 도메인을 등록하거나.
...
도메인 미등록이 이 모양으로 나타난다. `script` 실패와 구분해야 사람이 어디를 볼지 안다.
...
### UC-LM-SDK-008 실패한 뒤에는 다시 시도할 수 있다
...
- **Given** 실패 문구 표가 있고
- **When** 값들을 보면
- **Then** 넷이 서로 다르고, `init` 은 도메인을, `no-key` 는 변수 이름을 가리킨다
