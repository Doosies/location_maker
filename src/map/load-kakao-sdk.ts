 ...
 * Kakao 지도 SDK 를 붙인다.
 *
 * 실패 원인을 **구분해서** 돌려주는 것이 이 파일의 핵심이다. 셋 다 화면에서는
 * "지도가 안 뜬다" 로 같아 보이지만 사람이 할 일이 전혀 다르다 — 키를 넣거나,
 * 네트워크를 보거나, 콘솔에 도메인을 등록하거나.
 */

export type LoadFailure =
  | 'no-key' // 키가 주입되지 않았다 → .env.local 또는 Actions Secret
  | 'script' // 스크립트를 받지 못했다 → 네트워크·차단
  | 'timeout' // 받긴 했는데 초기화가 안 끝났다
  | 'init'; // 스크립트는 왔는데 kakao.maps 가 없다 → 도메인 미등록이 흔하다
...
export const LOAD_FAILURE_MESSAGE: Record<LoadFailure, string> = {
  ...
  script: '지도 스크립트를 받지 못했다. 네트워크를 확인하고 새로고침해 보자.',
  timeout: '지도 스크립트가 제때 응답하지 않았다. 새로고침해 보자.',
  init: '지도가 이 주소에서 거부됐다. Kakao 콘솔에 이 도메인이 등록됐는지 확인한다.',
};
  ...
  pending = new Promise<LoadResult>((resolve) => {
    ...
    script.addEventListener('load', () => {
      ...
      if (typeof load !== 'function') {
        // 스크립트는 왔는데 전역이 없다. 도메인이 등록되지 않은 경우가 대부분이다.
